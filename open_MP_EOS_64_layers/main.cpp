...
int main(int argc, char *argv[]) {
    ...
    //double TEMP46NDVI [64];
    double TEMP46NDVI_flipud [64];
    ...
    #pragma omp parallel for num_threads(num_threads) private(j,i,k,l,m,n,TEMP46NDVI_flipud,EXT09Data, EXT09DMSK2, LocMaxima,x,y,param,paramfit,pred_f)
    for ( j = 0; j < cols; ++j) {

        //omp_set_lock(&lock);
        //#pragma omp atomic
        for ( i = 0; i < rows; ++i) { // rows=46,
            TEMP46NDVI_flipud[i] = (float)pImageData[i * cols + j]/10000.0;
        }

        // flip up-down  NDVI
        //for (n = 0; n <46 ; ++n) {
        //    TEMP46NDVI_flipud=TEMP46NDVI;
        //}

        //NDVI46 to 365
        NDVI64to505(TEMP46NDVI_flipud, EXT09Data, EXT09DMSK2, &LocMaxima);
...
//    #pragma omp parallel for private(j)
    for (int j=0;j<cols;j++) {
        if ((short int)Date1[j]!=0){
            ...
            SOS6method[0*cols+j]=0;
            SOS6method[1*cols+j]=0;
            SOS6method[2*cols+j]=0;
            SOS6method[3*cols+j]=0;
            SOS6method[4*cols+j]=0;
            SOS6method[5*cols+j]=0;
        }
    }
